...
def tic():
  return time.time()
def toc(tstart, name="Operation", disp=False):
  dt = time.time() - tstart
  if disp:
    print('%s took: %s sec.\n' % (name, dt))
  return dt
...
def compute_stereo(left_img, right_img, disp=False):
  # path_l = 'data/image_left.png'
  # path_r = 'data/image_right.png'
  path_l = os.path.join('data/stereo_images/stereo_left', left_img)
  path_r = os.path.join('data/stereo_images/stereo_right', right_img)
  ...
  if disp:
    disparity_show = disparity / disparity.max()
    ...
    ax3.imshow(disparity_show, cmap='gray')
...
def test_mapCorrelation():
  ...
  # MAP['sizex']  = int(np.ceil((MAP['xmax'] - MAP['xmin']) / MAP['res'])) #cells
  ...
  # xis = np.ceil((xs0 - MAP['xmin']) / MAP['res'] ).astype(np.int16)
  # yis = np.ceil((ys0 - MAP['ymin']) / MAP['res'] ).astype(np.int16)
...
if __name__ == '__main__':
  compute_stereo(1,disp=True)
  # cv2.imshow('a',a)
  # show_lidar()
  # t = bresenham2D(49, 49, 49, 80)
  # test_mapCorrelation()
  # test_bresenham2D()
  print()
